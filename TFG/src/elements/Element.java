...
public class Element extends Actor {
	...
	protected float decelerationT = 50;
	protected float maxSpeedT = 500;
	...
	public Animation<TextureRegion> loadFullAnimationTrain(String name, int rows, int cols, float frameDuration,
			boolean loop) {
	...
	public void applyPhysicsTrain(float dt) {
		velocity.add(acceleration.x * dt, acceleration.y * dt);

		float speed = velocity.len();

		if (acceleration.len() == 0)
			speed -= decelerationT * dt;

		speed = MathUtils.clamp(speed, 0, maxSpeedT);
